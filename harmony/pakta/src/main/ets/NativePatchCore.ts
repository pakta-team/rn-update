/**
 * [INPUT]: 依赖 librnupdate.so NAPI 默认导出与共享 C++ state/patch/update-flow 方法签名
 * [OUTPUT]: 对外提供状态操作码、含成功确认匹配结果的 DTO、归档/请求接口和 NativePatchCore 收窄后的原生能力
 * [POS]: Harmony 编译期/运行时原生防腐层，ArkTS 只通过此文件消费共享内核，不解析 NAPI 细节
