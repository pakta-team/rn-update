/**
 * [INPUT]: 依赖 archive/digest/patch/state 共享内核、第三方 hpatch 测试产物与二进制 fixtures
 * [OUTPUT]: 提供宿主机集成测试，覆盖补丁格式、复制合并、路径安全、清理、版本绑定成功确认、状态机与摘要校验
 * [POS]: patch_core/tests 的主回归可执行程序，跨越规划与文件执行边界验证完整补丁链
...
using pakta::patch::ApplyPatchFromFileSource;
using pakta::patch::BundlePatcher;
using pakta::patch::CleanupOldEntries;
using pakta::patch::CopyOperation;
using pakta::patch::FileSourcePatchOptions;
using pakta::patch::PatchManifest;
using pakta::patch::Status;
using pakta::state::BinaryVersionSyncResult;
using pakta::state::LaunchDecision;
using pakta::state::MarkSuccessResult;
using pakta::state::State;
...
struct TempDir {
  ...
  TempDir() {
    char templ[] = "/tmp/pakta-patch-core-XXXXXX";
...
void TestApplyPatchMergeFallsBackToByteCopy() {
  ...
  pakta::patch::internal::g_disable_hard_links = true;
  Status status = ApplyPatchFromFileSource(options, patcher);
  pakta::patch::internal::g_disable_hard_links = false;
  Expect(status.ok, status.message);
...
void TestIsSafeRelativePathRejectsControlBytes() {
  using pakta::patch::IsSafeRelativePath;
  ...
  Expect(
      !pakta::patch::ValidateManifest(manifest).ok,
...
void TestStateCoreSyncBinaryVersionResetsUpdates() {
  ...
  BinaryVersionSyncResult result =
      pakta::state::SyncBinaryVersion(state, "1.1.0", "200");
...
void TestStateCoreSwitchVersionAndMarkSuccess() {
  ...
  State switched = pakta::state::SwitchVersion(state, "new");
  ...
  MarkSuccessResult success = pakta::state::MarkSuccess(switched, "new");
  Expect(success.accepted, "matching version should be marked successful");
...
void TestStateCoreRejectsStaleMarkSuccess() {
  State state;
  state.package_version = "1.0.0";
  state.build_time = "100";
  state.current_version = "new";
  state.last_version = "old";
  state.first_time = true;
  state.first_time_ok = false;
  state.rolled_back_version = "rolled";

  MarkSuccessResult stale = pakta::state::MarkSuccess(state, "old");
  Expect(!stale.accepted, "stale launch version must not confirm current version");
  ExpectEq(stale.state.current_version, "new", "stale confirmation changed current version");
  ExpectEq(stale.state.last_version, "old", "stale confirmation changed rollback version");
  Expect(stale.state.first_time, "stale confirmation cleared first_time");
  Expect(!stale.state.first_time_ok, "stale confirmation marked first_time_ok");
  ExpectEq(stale.state.rolled_back_version, "rolled", "stale confirmation changed rollback marker");
  Expect(stale.stale_version_to_delete.empty(), "stale confirmation scheduled version deletion");
}
...
void TestStateCoreResolveLaunchStateAndRollback() {
  ...
  LaunchDecision rollback =
      pakta::state::ResolveLaunchState(state, false, true);
  ...
  LaunchDecision consume =
      pakta::state::ResolveLaunchState(first_load, false, true);
  ...
  LaunchDecision preserve =
      pakta::state::ResolveLaunchState(first_load, false, false);
...
void TestStateCoreCanClearMarkers() {
  ...
  State clear_first_time = pakta::state::ClearFirstTime(state);
  ...
  State clear_rollback = pakta::state::ClearRollbackMark(state);
  Expect(
...
void TestArchivePatchCoreBuildPlanAndCopyGroups() {
  ...
  Status status = pakta::archive_patch::BuildArchivePatchPlan(
  ...
  status = pakta::archive_patch::BuildCopyGroups(manifest, &groups);
  Expect(status.ok, status.message);
  ...
  FileSourcePatchOptions options;
  status = pakta::archive_patch::BuildFileSourcePatchOptions(
      plan,
      "/tmp/source",
      "/tmp/target",
...
void TestArchivePatchCoreRejectsMissingEntries() {
  ...
  pakta::archive_patch::ArchivePatchPlan plan;
  status = pakta::archive_patch::BuildArchivePatchPlan(
      pakta::archive_patch::ArchivePatchType::kPatchFromPackage,
      manifest,
      {"__diff.json"},
      &plan);
  ...
  Expect(
      pakta::archive_patch::ClassifyEntry(
          pakta::archive_patch::ArchivePatchType::kPatchFromPackage,
          "__diff.json") == pakta::archive_patch::EntryAction::kSkip,
      "manifest entry should be skipped");
}

void TestTryParseArchivePatchType() {
  pakta::archive_patch::ArchivePatchType type;
  Expect(
      pakta::archive_patch::TryParseArchivePatchType(1, &type) &&
          type == pakta::archive_patch::ArchivePatchType::kFull,
      "1 should parse to kFull");
  Expect(
      pakta::archive_patch::TryParseArchivePatchType(2, &type) &&
          type == pakta::archive_patch::ArchivePatchType::kPatchFromPackage,
      "2 should parse to kPatchFromPackage");
  Expect(
      pakta::archive_patch::TryParseArchivePatchType(3, &type) &&
          type == pakta::archive_patch::ArchivePatchType::kPatchFromPpk,
      "3 should parse to kPatchFromPpk");
  Expect(
      !pakta::archive_patch::TryParseArchivePatchType(0, &type),
      "0 should be rejected");
  Expect(
      !pakta::archive_patch::TryParseArchivePatchType(4, &type),
...
void TestArchivePatchCoreHarmonyBundlePatchFromPackage() {
  ...
  pakta::archive_patch::ArchivePatchPlan ppk_plan;
  status = pakta::archive_patch::BuildArchivePatchPlan(
      pakta::archive_patch::ArchivePatchType::kPatchFromPpk,
      manifest,
      {"__diff.json", "bundle.harmony.js.patch", "assets/new.png"},
...
void TestStateCoreRollbackToEmptyVersion() {
  ...
  State rolled = pakta::state::Rollback(state);
...
void TestStateCoreResolveLaunchNoCurrentVersion() {
  ...
  LaunchDecision decision = pakta::state::ResolveLaunchState(state, false, true);
...
void TestStateCoreSwitchToSameVersion() {
  ...
  State switched = pakta::state::SwitchVersion(state, "same_hash");
...
void TestSha256File() {
  ...
  WriteFile(path, "abc");
  ExpectEq(
      pakta::digest::Sha256File(path),
      "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad",
      "Sha256File over a small file");
  ExpectEq(
      pakta::digest::Sha256File(dir + "/missing.bin"),
      "",
...
void TestCrc32KnownVectors() {
  ...
  }
  {
    ...
    pakta::digest::Crc32 one_shot;
    one_shot.Update(
        reinterpret_cast<const uint8_t*>(input.data()), input.size());
    pakta::digest::Crc32 streamed;
    size_t offset = 0;
    size_t chunk = 1;
...
void TestCrc32File() {
  ...
  uint32_t value = 0;
  Expect(pakta::digest::Crc32File(path, &value), "Crc32File should read");
  Expect(value == 0xCBF43926u, "Crc32File check value");
  Expect(
      !pakta::digest::Crc32File(dir + "/missing.bin", &value),
...
int main(int argc, char** argv) {
  ...
  const std::vector<std::pair<std::string, void (*)()>> tests = {
      ...
      {"StateCoreRejectsStaleMarkSuccess", TestStateCoreRejectsStaleMarkSuccess},
