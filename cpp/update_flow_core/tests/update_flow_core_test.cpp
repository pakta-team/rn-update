/**
 * [INPUT]: 依赖 flow_json、update_flow_core 与 TypeScript 生成的 flow_vectors.json
 * [OUTPUT]: 提供宿主机可执行测试，回放跨语言 golden 向量并覆盖解析器恶意输入和组合响应决策
 * [POS]: update_flow_core/tests 的协议同构证明，TypeScript 为参考实现、C++ 为被验证实现
