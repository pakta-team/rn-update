# [INPUT]: 依赖 R8/ProGuard、React Native/Expo 反射入口、JNI FindClass 结果类与本模块运行时类名
# [OUTPUT]: 对外提供消费者混淆保留规则，保证桥注册、JNI 字段访问、重载反射和更新任务名称稳定
# [POS]: android 发布优化边界，仅保护运行时可达性，不改变 Java 业务行为
