/**
 * [INPUT]: 依赖 bun:test mock.module 注入的 core 启动身份桩
 * [OUTPUT]: 验证 getUpdateMetadata 字段聚合、rescueSource 归类与标签/附加封装行为
 * [POS]: src/__tests__ 的元数据单元测试，保障崩溃报告可回溯到精确 bundle
...
describe('attachUpdateMetadata', () => {
  test('feeds Sentry-style reporters tags plus a context', async () => {
    ...
    expect(tags['pakta.currentVersion']).toBe('running-hash');
    expect(tags['pakta.bundleSha256']).toBe('c'.repeat(64));
    expect(tags['pakta.isFirstTime']).toBe('true');
    expect(tags['pakta.rescueSource']).toBe('');
    expect(contexts.pakta).toEqual(meta);
  });
  ...
  test('feeds Crashlytics-style reporters custom keys and swallows failures', async () => {
    ...
    expect(received?.['pakta.packageVersion']).toBe('2.3.4');
