...
> 标定层 syncNativeConfig、iOS RCTPaktaOrchestrator、Android
...
2. **多 root component** —— 启动了不含 `UpdateProvider` 的那个入口；而且是静默的（该入口若未构造 `Pakta`，连契约错误都报不出来）
...
已实施（四仓库,本地提交待推送）:pakta-server / pakta-server 决策层从
`binding.config.forceBoot` 合成进下发 config（灰度与全量两分支）,绑定列表
接口补 select config;pakta-admin / pakta-admin 发布菜单加"全量+强制启动
...
- **Harmony:不需要 crash hold——RNOH 的进程模型天然免疫(2026-08-13 模
  ...
  **不杀进程**:Hermes 跑在 RNOH 自管线程,异常不会重抛到 ArkTS 主线程触发
  ...
  杀进程")被实证推翻。
  注意两个鸿蒙特有的工程事实:PaktaTurboModule.cpp 的 C++ 方法表是 RNOH
