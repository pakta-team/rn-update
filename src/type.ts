/**
 * [INPUT]: 依赖 error 的稳定 UpdateErrorCode，并承载客户端与服务端约定的更新字段语义
 * [OUTPUT]: 对外提供版本、检查结果、事件、服务端配置、客户端选项、下载进度与测试载荷等共享类型
 * [POS]: src 的协议类型模型，被 client、Provider、纯决策与遥测共同依赖；不得反向依赖具体 UI 或 IO 实现
...
interface RootResult {
   ...
   * binary's bundleHash and buildTime are registered; 'rebuiltSameJs' — the
   * same JS fingerprint is registered but this buildTime is different or
   * absent, so only a full update is eligible; 'unknownBundle' — the JS
   * fingerprint is unregistered and strict channel delivery keeps the
   * current package unless the compatibility switch is enabled.
   */
...
export interface ClientOptions {
   ...
   * `__rnPaktaVersionHash` deep links / QR codes) that target a specific
...
export interface UpdateTestPayload {
  type: '__rnPaktaVersionHash' | string | null;
  data: any;
}
