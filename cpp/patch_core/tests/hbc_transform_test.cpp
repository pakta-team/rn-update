/**
 * [INPUT]: 依赖 hbc_transform、hbc_transform_wire 与 fixtures 中由 JS 参考实现生成的 HBC 样本
 * [OUTPUT]: 提供宿主机可执行测试，覆盖 golden 对拍、可逆性、非法布局拒绝与 wire 解析边界
 * [POS]: patch_core/tests 的 HBC 协议回归套件，证明生成端元数据与客户端解释器保持同构
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */
// hbc_transform 宿主机测试:
// 1) 与 JS 参考实现(rn-update-cli src/utils/hbcTransform.ts)
...
using pakta::hbc::BuildLayout;
using pakta::hbc::HbcDeltaField;
using pakta::hbc::HbcLayoutDesc;
using pakta::hbc::HbcSectionDesc;
using pakta::hbc::HbcTransformMeta;
using pakta::hbc::ParseHbcTransformMeta;
using pakta::hbc::TransformHbcInPlace;
