/**
 * [INPUT]: 依赖 OpenHarmony NAPI、共享 patch_core/state_core/update_flow_core、摘要/HBC 变换与 Android JNI 补丁适配头
 * [OUTPUT]: 对外导出状态同步/操作及成功确认匹配结果、归档规划/复制、异步补丁/清理、SHA-256/CRC32、diffVersion 和 update-flow JSON 函数
 * [POS]: Harmony NAPI 原生核心，负责 JS 值校验、线程切换和 Promise 结算，算法语义全部委托共享 C++
...
pakta::state::State ReadState(napi_env env, napi_value value, bool* ok) {
  if (ok != nullptr) {
    *ok = false;
  }
...
napi_value NewStateResult(
  ...
  SetBoolProperty(env, result, "markSuccessAccepted", mark_success_accepted);
  return result;
}
...
napi_value NewCopyGroupArray(
    napi_env env,
    const std::vector<pakta::archive_patch::CopyGroup>& groups) {
  napi_value result = nullptr;
...
napi_value SyncStateWithBinaryVersion(napi_env env, napi_callback_info info) {
  ...
  const pakta::state::State state = ReadState(env, args[2], &ok);
  if (!ok) {
    return nullptr;
  }

  const pakta::state::BinaryVersionSyncResult result =
      pakta::state::SyncBinaryVersion(state, package_version, build_time);
...
napi_value RunStateCore(napi_env env, napi_callback_info info) {
  ...
  bool ok = false;
  const pakta::state::State state = ReadState(env, args[1], &ok);
  if (!ok) {
    return nullptr;
  }
  ...
  switch (static_cast<StateOperation>(operation)) {
    case StateOperation::kSwitchVersion:
      return NewStateResult(
          env,
          pakta::state::SwitchVersion(state, string_arg),
          false,
          std::string(),
          std::string(),
          false,
    ...
    case StateOperation::kMarkSuccess: {
      const pakta::state::MarkSuccessResult result =
          pakta::state::MarkSuccess(state, string_arg);
      return NewStateResult(
          env,
          result.state,
          false,
    ...
    case StateOperation::kClearFirstTime:
      return NewStateResult(
          env,
          pakta::state::ClearFirstTime(state),
          false,
          std::string(),
          std::string(),
          false,
    ...
    case StateOperation::kClearRollbackMark:
      return NewStateResult(
          env,
          pakta::state::ClearRollbackMark(state),
          false,
          std::string(),
          std::string(),
          false,
    ...
    case StateOperation::kResolveLaunch: {
      const pakta::state::LaunchDecision decision =
          pakta::state::ResolveLaunchState(state, flag_a, flag_b);
      return NewStateResult(
          env,
          decision.state,
          false,
...
napi_value BuildArchivePatchPlan(napi_env env, napi_callback_info info) {
  ...
  const pakta::patch::PatchManifest manifest =
      BuildManifest(copy_froms, copy_tos, deletes);
  pakta::archive_patch::ArchivePatchType archive_type;
  if (!pakta::archive_patch::TryParseArchivePatchType(patch_type, &archive_type)) {
    ThrowError(env, "Unknown archive patch type");
    return nullptr;
  }
  pakta::archive_patch::ArchivePatchPlan plan;
  const pakta::patch::Status status = pakta::archive_patch::BuildArchivePatchPlan(
      archive_type,
      manifest,
      entry_names,
      &plan,
...
// that can take hundreds of ms to seconds. The Pakta TurboModule executes on
...
struct ApplyPatchWork {
  ...
  pakta::patch::Status status{false, ""};
};
...
napi_value ApplyPatchFromFileSource(napi_env env, napi_callback_info info) {
  ...
  if (napi_create_async_work(
          ...
          [](napi_env, void* data) {
            ...
            w->status = pakta::patch::ApplyPatchFromFileSource(w->options);
          },
...
napi_value CleanupOldEntries(napi_env env, napi_callback_info info) {
  ...
  if (napi_create_async_work(
          ...
          [](napi_env, void* data) {
            ...
            w->status = pakta::patch::CleanupOldEntries(
...
napi_value Sha256HexFileAsync(napi_env env, napi_callback_info info) {
  ...
  if (napi_create_async_work(
          ...
          [](napi_env, void* data) {
            ...
            w->hex = pakta::digest::Sha256File(w->path);
          },
          [](napi_env cb_env, napi_status status, void* data) {
            auto* w = static_cast<Sha256FileWork*>(data);
            if (status != napi_ok) {
...
static napi_value Sha256HexFile(napi_env env, napi_callback_info info) {
  ...
  return MakeUtf8String(env, pakta::digest::Sha256File(path));
}
...
static napi_value GetSupportedDiffVersion(napi_env env, napi_callback_info) {
  ...
  napi_create_uint32(
      env,
      static_cast<uint32_t>(pakta::hbc::kSupportedDiffVersion),
      &result);
  return result;
}
