/**
 * [INPUT]: 依赖 hdc/uitest 命令、Node child_process 与 Harmony 布局 JSON
 * [OUTPUT]: 对外提供 UiNode、WaitOptions 和带启动稳定窗口的 HarmonyDriver 设备交互原语
 * [POS]: Harmony e2e 测试与真实设备之间的唯一驱动适配层
...
function asRawLayoutNode(value: unknown): RawLayoutNode {
  if (!isRecord(value)) {
    return {};
  }
  return {
...
function flattenLayout(root: unknown): UiNode[] {
  const nodes: UiNode[] = [];
  const walk = (value: unknown) => {
    const node = asRawLayoutNode(value);
    const rawAttributes = node.attributes ?? {};
    const attributes: Record<string, string> = Object.fromEntries(
      Object.entries(rawAttributes).map(([key, attribute]) => [
        key,
        typeof attribute === 'string' ? attribute : String(attribute ?? ''),
      ])
    );
    const bounds = parseBounds(attributes.bounds);
    nodes.push({
      type: stringAttribute(rawAttributes, 'type'),
      id: stringAttribute(rawAttributes, 'id'),
      text: stringAttribute(rawAttributes, 'text'),
      bounds,
      center: bounds
        ? {
...
export class HarmonyDriver {
  ...
  async launch(): Promise<void> {
    ...
    // RNOH 的 uitest 广播服务在 Ability 成功返回后仍需短暂初始化；
    // 过早 dumpLayout 会卡在 subscribe timeout，并污染后续 hdc 调用。
