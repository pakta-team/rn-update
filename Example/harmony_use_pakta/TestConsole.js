/**
 * [INPUT]: 依赖 rn-update 的 PaktaModule 与 full/ppk/package 调试制品参数。
 * [OUTPUT]: 对外提供 TestConsole 组件，将文本命令转成原生更新方法并显示执行结果。
 * [POS]: 手工诊断面板，被 App.tsx 按需挂载，不持有 Provider 的更新状态。
...
const AppPatchDownloadUrl =
  'https://github.com/bozaigao/test_pakta_server/raw/refs/heads/main/hdiff.app-patch';
const AppPatchHash = 'f5ba92c7c04250d4b8a446c8267ef459';
const PPKDownloadUrl =
  'https://github.com/bozaigao/test_pakta_server/raw/refs/heads/main/hdiff.ppk-patch';
      ...
      <TouchableOpacity
        ...
        onPress={async () => {
          ...
          try {
            ...
            if (inputs.length === 1) {
              if (options) {
                await PaktaModule[methodName](options);
              } else {
                await PaktaModule[methodName]();
              }
            } else {
              if (inputs.length === 2) {
                params = [inputs[1]];
              ...
              await PaktaModule[methodName](...params);
            }
