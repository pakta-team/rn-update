/**
 * [INPUT]: 依赖 flow_json.h 的有序值模型与严格 JSON/JavaScript 序列化约定
 * [OUTPUT]: 实现对象访问、JS truthiness/严格相等、受深度限制的解析与规范化序列化
 * [POS]: flow_json 的确定性实现，为网络响应解析和 TS/C++ golden 对拍提供共同数据语义
