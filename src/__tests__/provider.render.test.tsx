/**
 * [INPUT]: 依赖 React、react-test-renderer、setup.ts 的 Alert/AppState 替身，动态导入 ../provider 与 ../context，并注入可控 client、检查结果和进度事件
 * [OUTPUT]: 提供 UpdateProvider 的提示策略、下载/切换动作、错误态、AppState 恢复与进度消费者隔离的渲染交互契约
 * [POS]: __tests__ 的 Provider 高层集成套件，验证 client 事件如何投影为 React 状态和 UI 副作用，连接 context 边界回归
...
const createClient = (options: Record<string, any> = {}) => {
  ...
  const client = {
    ...
    // The provider's option-change contract (see Pakta.setOptions): options
  ...
  test('test-channel payloads are refused when testChannel is false', async () => {
    ...
    await TestRenderer.act(async () => {
      handled = captured.current.parseTestQrCode(
        JSON.stringify({ type: '__rnPaktaVersionHash', data: 'target-hash' })
      );
      await flush();
    });
