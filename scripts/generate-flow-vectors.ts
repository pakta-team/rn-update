/**
 * [INPUT]: 依赖 ../src/updateFlowCore 的无 IO 参考函数与 Bun 文件写入能力，使用覆盖 hash、rollout、端点、请求和下载策略的输入矩阵
 * [OUTPUT]: 导出 FlowVector/buildVectors，并在直接执行时生成 cpp/update_flow_core/tests/flow_vectors.json 黄金向量
 * [POS]: scripts 的跨语言语义生成器，连接 TypeScript 决策核心与 C++ 回放测试；生成 JSON 是唯一副作用
