/**
 * [INPUT]: 依赖绕过 setup.ts mock 的 ../i18n 真实实现，以及中英文翻译表和插值参数
 * [OUTPUT]: 提供 locale 读写、双向回退、字符串插值、运行时扩展与 singleton 导出的稳定性回归
 * [POS]: __tests__ 的无 IO 国际化纯模块套件，锁定文案解析边界并与 client/provider 的消费者解耦
