/**
 * [INPUT]: 依赖 Android Context/SharedPreferences/包信息、DownloadTask 单线程队列、patch_core 状态机 JNI 与原生检查编排器
 * [OUTPUT]: 对外提供 UpdateContext 单例、下载/切包、按启动版本身份确认成功、回滚/重置、bundle URL/hash、KV 与提交代际能力
 * [POS]: Android 更新聚合上下文和持久状态权威，协调磁盘事务、启动选择与救砖提交，桥层只通过它发起用例
...
public class UpdateContext {
    ...
    private static final String KEY_BINARY_CHANNEL = "binaryChannel";
    ...
    private UpdateContext(Context context) {
        ...
        this.executor = Executors.newSingleThreadExecutor(new ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
                return new Thread(r, "pakta-download");
            }
        });
        ...
        String previousChannel = sp.getString(KEY_BINARY_CHANNEL, null);
        boolean channelChanged = previousChannel != null && !previousChannel.equals(channel);
        ...
        if (channelChanged) {
            // 渠道是安装包身份的一部分。覆盖安装另一渠道时，只撤销热更
            // 状态与响应缓存，保留 uuid/nativeConfig 等跨渠道安装级数据。
            resetGeneration.incrementAndGet();
            nextState = new StateCoreResult();
            nextState.packageVersion = packageVersion;
            nextState.buildTime = buildTime;
            nextState.firstTime = false;
            nextState.firstTimeOk = true;
            nextState.changed = true;
        }

        if (nextState.changed || channelChanged) {
            ...
            editor.putString(KEY_BINARY_CHANNEL, channel);
            persistEditor(editor, "sync state with binary version");
        } else if (previousChannel == null) {
            sp.edit().putString(KEY_BINARY_CHANNEL, channel).apply();
        }
    }
    ...
    public String getBuildTime() {
        return context.getString(R.string.pakta_build_time);
    }

    /** Reads the immutable distribution channel from the host manifest. */
    public String getChannel() {
        try {
            android.content.pm.ApplicationInfo info = context.getPackageManager()
                .getApplicationInfo(context.getPackageName(), PackageManager.GET_META_DATA);
            if (info.metaData != null) {
                String value = info.metaData.getString("channel");
                if (value != null && !value.trim().isEmpty()) {
                    return value.trim();
                }
            }
        } catch (PackageManager.NameNotFoundException ignored) {
            Log.w(TAG, "Unable to read channel from host manifest");
        }
        return "default";
    }
     ...
     * pakta::digest: librnupdate.so is a prebuilt artifact and this must not
...
//        params.unzipDirectory = new File(rootDir, hash);
    ...
    public boolean markSuccess(String expectedHash) {
        ...
        synchronized (commitLock) {
            StateCoreResult currentState = getStateSnapshot();
            // expectedHash 还必须是本进程实际解析并加载的 bundle 版本。
            // 仅比较持久化 currentVersion 会把冷启动检查在 JS 初始化前切换的版本误报成功。
            if (expectedHash == null || launchVersion == null || !expectedHash.equals(launchVersion)) {
                Log.w(TAG, "markSuccess rejected before state core: expected=" + expectedHash
                    + ", launch=" + launchVersion
                    + ", current=" + currentState.currentVersion
                    + ", firstTime=" + currentState.firstTime
                    + ", firstTimeOk=" + currentState.firstTimeOk);
                return false;
            }
            StateCoreResult nextState = runStateCore(
            ...
            if (!nextState.markSuccessAccepted) {
                Log.w(TAG, "markSuccess rejected by state core: expected=" + expectedHash
                    + ", launch=" + launchVersion
                    + ", current=" + currentState.currentVersion
                    + ", firstTime=" + currentState.firstTime
                    + ", firstTimeOk=" + currentState.firstTimeOk);
