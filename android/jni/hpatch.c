/*
 * [INPUT]: 依赖 hpatch.h、HDiffPatch 文件流 API 与 LZMA/LZMA2 解压插件
 * [OUTPUT]: 对外实现 hpatch_by_file，自动识别 HDIFFSF20 single 与 HDIFF13 stream 补丁并返回稳定错误码
 * [POS]: jni 自有补丁格式适配器，为共享 patch_core 提供有界内存的 Android 文件变换能力
