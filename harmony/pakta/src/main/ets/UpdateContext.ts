/**
 * [INPUT]: 依赖 Harmony preferences/fs/bundle/resource APIs、DownloadTask、NativePatchCore、NativeCheckOrchestrator、JS 冻结的启动版本身份与路径校验
 * [OUTPUT]: 对外提供单例 UpdateContext，读取不可变 channel，管理持久状态、串行任务、bundle 解析、下载/补丁、跨渠道撤销、切包/回滚/重置、版本绑定成功确认和 bundleHash
 * [POS]: Harmony 更新聚合根与状态权威，协调 RNOH bundle provider、TurboModule 和无 JS 检查的共享生命周期
...
export class UpdateContext {
  ...
  private static cachedChannel: string = '';
  ...
  private constructor(context: common.UIAbilityContext) {
    ...
    const previousChannel = this.readString('binaryChannel');
    const channelChanged =
      previousChannel.length > 0 && previousChannel !== channel;
    this.syncStateWithBinaryVersion(packageVersion, buildTime, channelChanged);
    if (channel) {
      this.preferences.putSync('binaryChannel', channel);
      this.flushInBackground('sync binary channel');
    }
  }

  /**
   * 诊断日志：打印本实例 id 与关键状态，用于定位 preferences 多实例 / 状态分裂问题。
   * 通过 hilog 输出，prefix=pakta，可在 hilog 中按 "UpdateContext" 过滤。
   */
  ...
  public getBuildTime(): string {
    ...
    try {
      ...
      if (metaData.pakta_build_time) {
        UpdateContext.cachedBuildTime = String(metaData.pakta_build_time);
        return UpdateContext.cachedBuildTime;
      }
    } catch (error) {
      logger.error(
  ...
  /**
   * 读取宿主 rawfile/meta.json 中固定的原生渠道。渠道属于安装包身份，
   * 不接受 JS 配置覆盖；未配置时显式归入 default。
   */
  public getChannel(): string {
    if (UpdateContext.cachedChannel) {
      return UpdateContext.cachedChannel;
    }
    try {
      const content =
        this.context.resourceManager.getRawFileContentSync('meta.json');
      ...
      if (typeof metaData.channel === 'string') {
        const value = metaData.channel.trim();
        if (value) {
          UpdateContext.cachedChannel = value;
          return value;
        }
      }
    } catch (error) {
      logger.error(
        TAG,
        `Failed to read channel from raw file: ${getErrorMessage(error)}`,
      );
    }
    UpdateContext.cachedChannel = 'default';
    return UpdateContext.cachedChannel;
  }
  ...
  public syncStateWithBinaryVersion(
    ...
    if ((!packageVersion || !buildTime) && !forceReset) {
      return;
    }
    const currentState = this.getStateSnapshot();
    const nextState = forceReset
      ? {
          packageVersion: packageVersion || currentState.packageVersion,
          buildTime: buildTime || currentState.buildTime,
          currentVersion: '',
          lastVersion: '',
          firstTime: false,
          firstTimeOk: true,
      ...
      : NativePatchCore.syncStateWithBinaryVersion(
          packageVersion,
          buildTime,
          currentState,
        );
    ...
    if (forceReset) {
      ...
      if (typeof api.getAllSync === 'function') {
        try {
          ...
          logger.error(
            TAG,
            `Failed to clear channel hash info: ${getErrorMessage(e)}`,
          );
        }
      }
  ...
  public async markSuccess(expectedHash: string): Promise<boolean> {
    ...
    if (!UpdateContext.launchVersion || expectedHash !== UpdateContext.launchVersion) {
      return false;
    }
