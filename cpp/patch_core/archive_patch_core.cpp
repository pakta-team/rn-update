/**
 * [INPUT]: 依赖 archive_patch_core.h 的归档模型与 patch_core 的 manifest 校验能力
 * [OUTPUT]: 实现归档类型解析、条目分类、补丁计划、复制分组与文件源参数构建
 * [POS]: archive_patch_core 的纯规划实现，位于平台解包层与 patch_core 文件执行器之间
