/**
 * [INPUT]: 依赖 update_flow_core.h、flow_json 的 JS 兼容语义与 TypeScript golden 向量约束
 * [OUTPUT]: 实现 Murmur3 灰度、端点排序、check 请求/响应、下载尝试和 forceBoot 激活决策
 * [POS]: update_flow_core 的无 IO 策略实现，与 src/updateFlowCore.ts 逐行为同构
...
Value BuildCheckRequestBody(const Value& input) {
  ...
  // Native callers provide the immutable channel. Keep the same default as
  // the TypeScript reference when an older native bridge omits the field.
  const Value& channel = input.Get("channel");
  if (channel.IsString() && !channel.AsString().empty()) {
    body.Set("channel", channel);
  } else {
    body.Set("channel", Value::String("default"));
  }
