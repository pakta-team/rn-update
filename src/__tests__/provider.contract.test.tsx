/**
 * [INPUT]: 依赖真实 Pakta 客户端（仅 mock 原生模块与 core 常量）、context 的 useUpdate、react-test-renderer 与 setup 工具
 * [OUTPUT]: 验证 Provider 与真实客户端之间的成员契约及更新/激活流程交互
 * [POS]: src/__tests__ 的契约测试，守护手写 fake 客户端与真实客户端行为一致
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */
/**
 * Provider <-> client contract, exercised with the REAL Pakta client (only
...
const setupRealClient = async (
  ...
  const { Pakta } = await import(`../client?contract-${seq}`);
  const { UpdateProvider } = await import(`../provider?contract-${seq}`);
  const client = new Pakta({ appKey: 'contract-app', ...options });
...
describe('UpdateProvider <-> Pakta contract', () => {
  beforeEach(() => {
    mockAlert.mockClear();
