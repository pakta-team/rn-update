/**
 * [INPUT]: 依赖 ../updateFlowCore 的请求体构造、端点候选排序和下载决策函数，使用 CheckResult、设备 cInfo、灰度与补丁产物夹具
 * [OUTPUT]: 提供无 IO 更新流契约，锁定 dev/生产请求字段、diff→pdiff→full 策略、URL 合并和端点顺序
 * [POS]: __tests__ 的核心纯决策套件，作为 client 编排与 JS/C++ 黄金向量之间的语义基准
