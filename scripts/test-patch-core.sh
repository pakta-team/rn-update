#!/bin/sh
# [INPUT]: 依赖 C/C++ 编译器、cpp/patch_core 与 Android HDiffPatch/LZMA 源码和 tests/fixtures，可选 SANITIZE=1 检测内存/未定义行为
# [OUTPUT]: 在 .tmp/patch-core-tests 编译并运行补丁核心、HBC 变换两个独立测试二进制，以退出码表达回归结果
# [POS]: scripts 的补丁核心验证入口，复用原生实现和固定夹具，不参与 npm 运行时构建
