/**
 * [INPUT]: 依赖 core 冻结的启动身份（cInfo/packageVersion/buildTime/currentVersion 等安装记录）
 * [OUTPUT]: 提供 getUpdateMetadata、updateMetadataTags、attachUpdateMetadata 及 Sentry/Crashlytics 便捷封装
 * [POS]: src 的崩溃报告元数据桥，把运行中 bundle 身份映射到外部崩溃平台的 tag/context
...
/** Flat string tags (`pakta.currentVersion` …) for the running update. */
...
export function attachUpdateMetadata(
  ...
  const tags = updateMetadataTags(metadata, options.prefix ?? 'pakta.');
  ...
  if (
    ...
    swallow(() =>
      reporter.setContext?.(options.contextName ?? 'pakta', { ...metadata })
    );
  }
  return metadata;
}

/** `attachUpdateMetadata` for `@sentry/react-native` (tags + a `pakta` context). */
export function attachToSentry(sentry: CrashReporterLike): UpdateMetadata {
  return attachUpdateMetadata(sentry, { contextName: 'pakta' });
}
