/**
 * [INPUT]: 依赖 JNI、patch_core/jni_util、flow_json 与 update_flow_core 的纯决策接口
 * [OUTPUT]: 对 Android NativeUpdateFlow 暴露请求体构建、端点排序和响应处理的 JSON JNI 入口
 * [POS]: update_flow_core 的 Android 字符串边界，负责 UTF-8/UTF-16 与 JSON 映射而不承载策略
...
Java_cn_reactnative_modules_update_NativeUpdateFlow_buildCheckRequestBody(
  ...
  try {
    ...
    flowjson::Value input = flowjson::Parse(
        pakta::jni_util::JStringToString(env, inputJson), &ok);
...
Java_cn_reactnative_modules_update_NativeUpdateFlow_orderEndpointCandidates(
  ...
  try {
    ...
    flowjson::Value endpoints = flowjson::Parse(
        pakta::jni_util::JStringToString(env, endpointsJson), &ok);
...
Java_cn_reactnative_modules_update_NativeUpdateFlow_isValidCheckResponse(
  ...
  try {
    return updateflow::IsValidCheckResponse(
               pakta::jni_util::JStringToString(env, responseText))
               ? JNI_TRUE
               : JNI_FALSE;
  } catch (...) {
    return JNI_FALSE;
...
Java_cn_reactnative_modules_update_NativeUpdateFlow_handleCheckResponse(
  ...
  try {
    ...
    flowjson::Value identity = flowjson::Parse(
        pakta::jni_util::JStringToString(env, identityJson), &ok);
    ...
    return ToJString(env, flowjson::Stringify(updateflow::HandleCheckResponse(
                              pakta::jni_util::JStringToString(env, responseText),
                              identity, false,
                              pakta::jni_util::JStringToString(env, afterDownload))));
  } catch (...) {
    return nullptr;
  }
}
