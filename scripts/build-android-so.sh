#!/usr/bin/env bash
# [INPUT]: 依赖 ANDROID_NDK_HOME/ROOT、ANDROID_HOME 与 RNUPDATE_NDK_VERSION 环境，读取 android/jni 的 NDK 工程和共享原生源码
# [OUTPUT]: 通过 ndk-build 生成 android/lib 下四个 ABI 的 librnupdate.so，并调用 verify-android-so.js 做发布前符号校验
# [POS]: scripts 的 Android 原生产物构建入口，只编排既有 Android.mk/Application.mk，不承载补丁业务逻辑
