/**
 * [INPUT]: 依赖 UpdateContext、ReadableMap/Promise、UpdateModuleSupport、StateSerialRunner、UiThreadRunner 与 ReactReloadManager
 * [OUTPUT]: 对外提供新旧架构共用的下载、安装、切包、重载、版本绑定成功确认、重置、配置、缓存和本地版本信息静态命令
 * [POS]: Android 原生模块应用服务门面，统一参数提取、线程归属与稳定错误码，防止两套桥实现行为漂移
...
/**
 * Everything the "Pakta" native module does, independent of the bridge it is
...
public class UpdateModuleImpl {
    ...
    public static final String NAME = "Pakta";
    ...
    public Map<String, Object> getConstants() {
        ...
        constants.put("channel", updateContext.getChannel());
        ...
        try {
            ...
            Log.e("pakta", "supportedDiffVersion missing from librnupdate.so", e);
        }
    ...
    public void markSuccess(final String expectedHash, final Promise promise) {
        StateSerialRunner.run(promise, ErrorCodes.MARK_SUCCESS_FAILED, "markSuccess", new StateSerialRunner.Operation() {
            @Override
            public void run() {
                promise.resolve(updateContext.markSuccess(expectedHash));
            }
        });
    }
