/**
 * [INPUT]: 仅以 type-only 方式依赖 CheckResult，所有运行时输入由调用参数注入
 * [OUTPUT]: 对外提供 rollout/hash、URL 与端点规则、检查请求/结果、激活判断及下载策略的纯函数和类型
 * [POS]: src 的无 IO 领域决策核心，被 client/Provider 调用并通过共享向量与 C++ update_flow_core 保持行为同构
...
export interface CheckRequestInput {
  ...
  /** Immutable channel declared by the native binary. */
  channel?: string;
