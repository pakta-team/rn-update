/**
 * [INPUT]: 依赖 Bun mock/afterEach 生命周期，注入 react-native、i18n、ReactNativeVersion 的基础替身，并预加载 core、endpoint、error、permissions、telemetry、updateFlowCore、utils 真实导出
 * [OUTPUT]: 对测试套件提供 mockAlert、emitAppStateChange 与全局 fetch/模块隔离；每例后恢复可复用的 React Native 与项目叶模块环境
 * [POS]: __tests__ 的共享测试基础设施，被 Provider/灰度等套件消费；它只维护隔离边界，不属于生产运行时
