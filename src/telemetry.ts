/**
 * [INPUT]: 依赖 error 的 UpdateErrorCode 与 type 的本地 EventType，并遵守服务端事件枚举和详情长度协议
 * [OUTPUT]: 对外提供 ServerEventType、ServerReportPayload（含 channel/buildTime）、MAX_DETAIL_LENGTH，以及事件类型/hash/详情的纯映射函数
 * [POS]: src 的遥测防腐层，将细粒度本地生命周期压缩为服务端可聚合语义，由 client 在发送回执前消费
...
export interface ServerReportPayload {
  ...
  channel?: string;
  /** Opaque native build identity; never coerce this to a number. */
  buildTime?: string;
  cInfo?: {
    rnu?: string;
    rn?: string;
