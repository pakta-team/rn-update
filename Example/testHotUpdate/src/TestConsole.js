/**
 * [INPUT]: 依赖 PaktaModule 原生门面、Platform 地址规则和本地 full/ppk/package 制品常量。
 * [OUTPUT]: 对外提供 TestConsole 组件，将文本命令解析为可执行的原生更新操作并显示结果。
 * [POS]: JS 诊断层，被 index.tsx 长按入口挂载；不拥有更新状态，只转发给 SDK 原生实现。
...
import { PaktaModule } from 'rn-update/src/core';
...
export default function TestConsole({ visible, onClose }) {
  ...
  const invokePaktaMethod = async (methodName, params) => {
    if (methodName === 'setLocalHashInfo') {
      ...
      return PaktaModule.setLocalHashInfo(hash, JSON.stringify(info));
    }

    if (params === undefined) {
      return PaktaModule[methodName]();
    }

    return PaktaModule[methodName](params);
  };
  const shortCuts = [
    {
  ...
  return (
    <Modal visible={visible}>
      <SafeAreaView style={{ flex: 1, padding: 10 }}>
        ...
        >
          <Text>调试Pakta方法（方法名，参数，值换行）</Text>
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            ...
            try {
              ...
              if (
                !methodName ||
                typeof PaktaModule[methodName] !== 'function'
              ) {
              ...
              let output;
              if (inputs.length === 1) {
                output = await invokePaktaMethod(methodName);
              } else {
                if (inputs.length === 2) {
                  params = inputs[1];
                } else {
                ...
                output = await invokePaktaMethod(methodName, params);
              }
              const message =
                output == null
                  ? ''
