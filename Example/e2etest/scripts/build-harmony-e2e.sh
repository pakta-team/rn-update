#!/usr/bin/env bash
###############################################################################
# [INPUT]: 依赖 Harmony hvigor、本地 HAR、RN e2etest 与 hdc 工具链
# [OUTPUT]: 对外提供 Harmony 基础应用构建/安装命令
# [POS]: e2etest 跨平台构建编排脚本，只串联既有工具
# [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
###############################################################################
# Build & install the HarmonyOS e2e base app, encoding the required order:
#   local har -> swap local rn-update -> refresh oh_modules
#   -> prepare update artifacts -> base bundle -> assembleHap -> install.
#
# The order matters: `pakta bundle --platform harmony` (run by the prepare
...
#   RNU_CLI_ROOT             rn-update-cli checkout (else sibling dir)
#   RNU_HARMONY_BUILD_MODE   release (default) | debug
#   SKIP_HAR=true            skip rebuilding pakta.har + oh_modules refresh
...
HARMONY_APP="$REPO_ROOT/Example/harmony_use_pakta"
...
# Release by default: debug haps skip markSuccess by design, so a debug hap
# rolls every update back on the next launch.
BUILD_MODE="${RNU_HARMONY_BUILD_MODE:-release}"
OHPM="$DEVECO_HOME/tools/ohpm/bin/ohpm"
BUNDLE_NAME="${RNU_HARMONY_BUNDLE_NAME:-com.charmlot.testpakta}"
...
if [ "${SKIP_HAR:-false}" != "true" ]; then
  echo "==> Building local pakta.har"
  (cd "$REPO_ROOT" && npm run build:harmony-har)

  echo "==> Syncing local rn-update into harmony_use_pakta"
  rsync -a --delete \
    --exclude node_modules --exclude Example --exclude .git \
    --exclude e2e --exclude .pakta \
    "$REPO_ROOT/" "$HARMONY_APP/node_modules/rn-update/"
