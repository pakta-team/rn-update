...
| 项目 | 结论 | 落地方式 |
|---|---|---|
| P1 Android/Harmony 重复下载 | 已修复 | 任务真正开始时复查 `.pakta-complete` + bundle；失败清理检测到完整安装时不删目录。iOS 注册表同时升级为按 hash + artifact type 合流/排队 |
...
| P3 `.pakta-complete` 迁移 | 接受一次性重下 | 不把仅有 bundle 的旧目录推断为完整安装，避免把半解压目录误标为成功；10.51.0 发布说明仍需明确这一流量成本 |
...
| 9 iOS 完成判定重复 | 已修复 | 抽取 `PaktaHasCompletedVersionAtPath`，预检与冷启动编排器共用同一 bundle+marker 判定 |
...
8. **iOS 合流者进度块重复发事件**(N 合流者 = N 倍 RN 事件,事件发送应只由
   owner 承担)。
9. **完成判定(bundle+marker)在 iOS 两处手写**,抽 PaktaHasCompletedVersion
   helper 防漂移。
...
（`PaktaRegisterDownload`/`PaktaFinishDownload`）,同一 hash 的并发下载合流。
...
写入 `.pakta-complete` 并 `setNeedUpdate(H)` 后,排队的重复任务重新下载 H
...
2. 更小的止血:`cleanUpAfterFailure` 里若目标目录已有
   `.pakta-complete` 标记则不删(失败的是重复下载,不是这份安装);
...
## P3 — `.pakta-complete` 无迁移,存量已下载版本升级后重下一次
...
端选取:iOS 复用既有 `PaktaWithStateLock`(为此把 switchVersion 拆出无锁核
`PaktaSwitchVersionLocked`,避免不可重入死锁)、Android 用共享
...
- [ ] 服务端已推送未激活:pakta 主机 `install-npm-release.sh` +
      `service-cli.sh restart all`(验 serverVersion);pakta 跑
...
DevEco api20 模拟器(HarmonyOS 6.0 / arm64)+ harmony_use_pakta(RN 0.72 +
RNOH 0.72.96)+ 本地 e2e 服务端。探针脚手架入库:
`harmony_use_pakta/e2e/entry.crashprobe.ts`(+1s 未捕获 JS 错误)、
...
**功能性断裂(已修,severity 高):`PaktaTurboModule.cpp` 的 C++ 方法表漏
