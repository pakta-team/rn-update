# rn-update — HarmonyOS module (`pakta`)

Integration notes for the HAR that ships in the `rn-update` npm
package (`harmony/pakta.har`). The general setup lives in the project README;
...
import { reactNativeUpdatePlugin } from 'pakta/hvigor-plugin';
...
It writes `src/main/resources/rawfile/meta.json` (`pakta_build_time`,
`versionName`) into that module during `assemble*` builds only, before
resources are compiled. Debug builds get `pakta_build_time: "0"` like Android,
so a debug rebuild does not look like a new binary. Add the generated file to
`.gitignore`.

## Versioning

`oh-package.json5` `version` and `BuildProfile.ets` `HAR_VERSION` mirror the npm
package version; `scripts/build-harmony-har.js` rewrites both before
`assembleHar`, and `scripts/check-release-version.js` asserts the committed
values match.

This file is packaged with the npm module (`harmony/pakta/src/`) so it is
available next to the sources in `node_modules/rn-update`.
