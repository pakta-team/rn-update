/**
 * [INPUT]: 依赖 Bun 模块替身与 console spy，注入 react-native NativeModules、ReactNativeVersion、nanoid 的版本与原生桥数据
 * [OUTPUT]: 提供 currentVersionInfo 合法/非法 JSON 解析、错误记录与原生版本元数据初始化的回归断言
 * [POS]: __tests__ 的 core 原生桥解析套件，锁定 JS 侧读取 TurboModule/旧桥常量时的容错边界
...
describe('web platform', () => {
  ...
  test('constants are explicit empties, no native calls, no rollback telemetry (1.1)', async () => {
    // On web PaktaModule is a Proxy of noops; reading the constants off it
    ...
    try {
      ...
      const { Pakta } = await importFreshClient('web-constants');
      const client = new Pakta({ appKey: 'web-app' });
