/**
 * [INPUT]: 依赖 HarmonyDriver、本地更新端口与测试设备上的 e2e bundle
 * [OUTPUT]: 对外提供 Harmony full/ppk/v2-track 更新流、健康确认和 up-to-date 端到端断言
 * [POS]: e2e/harmony 的主业务验证套件，不实现 SDK 更新逻辑
 ...
 * - The harmony base hap built from Example/harmony_use_pakta with the e2e
...
async function waitForEvent(expectedEvent: string, timeout = READY_TIMEOUT) {
  const expectedText = `lastEvent: ${expectedEvent}`;
  await driver.waitFor(
    async () => {
      const node = await driver.findById('last-event');
      return node?.text === expectedText ? node : undefined;
    },
    { timeout, description: `last event "${expectedEvent}"` }
  );
}

async function waitForCheckResult(
  expectedResult: string,
  timeout = RELOAD_TIMEOUT
) {
  const expectedText = `lastCheckResult: ${expectedResult}`;
  await driver.waitFor(
    async () => {
      const node = await driver.findById('last-check-result');
      return node?.text === expectedText ? node : undefined;
    },
    { timeout, description: `check result "${expectedResult}"` }
  );
}
...
describe('harmony local update flow', () => {
  it('starts from the binary base bundle after a data reset', async () => {
    ...
    // Reset pakta state so the run is deterministic regardless of what a
  ...
  it('applies the full package update (v1)', async () => {
    ...
    await waitForEvent('markSuccess');
  });
  ...
  it('reports up to date without switching bundles', async () => {
    await driver.tapById('check-update', { timeout: READY_TIMEOUT });
    await waitForCheckResult('upToDate');
    await waitForBundleLabel(LABELS.v2Track, READY_TIMEOUT);
  });
});
