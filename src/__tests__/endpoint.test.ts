/**
 * [INPUT]: 依赖 ../endpoint 的端点去重、回退执行与最快成功选择函数，使用确定性随机数、延迟、失败和 AbortSignal 替身
 * [OUTPUT]: 提供端点排序后的 hedge、超时、失败剪枝、重试、胜者取消与失败聚合契约
 * [POS]: __tests__ 的多端点纯策略回归，验证 endpoint 层而不触发真实网络，供 client 的端点编排复用
