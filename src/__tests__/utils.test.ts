/**
 * [INPUT]: 依赖 ../utils 的 URL、进度、超时 fetch、增强 HTTP 回退和 promiseAny 工具，注入 react-native Platform、i18n 与可控 fetch/AbortController
 * [OUTPUT]: 提供跨平台工具边界回归，锁定 URL 拼接、进度裁剪、超时取消、幂等 HTTP 降级和 Promise 聚合语义
 * [POS]: __tests__ 的横切基础工具套件，为 client 与 updateFlowCore 提供无真实网络的行为基准
...
describe('parseQueryParams', () => {
  test('parses the test-channel deep link', () => {
    expect(
      parseQueryParams('pakta://open?type=__rnPaktaVersionHash&data=abc123')
    ).toEqual({ type: '__rnPaktaVersionHash', data: 'abc123' });
  });

  test('a url without a query has no params', () => {
    expect(parseQueryParams('pakta://open')).toEqual({});
    expect(parseQueryParams('pakta://open?')).toEqual({});
