#!/usr/bin/env node
/**
 * [INPUT]: 依赖 fs/path 读取 cpp/patch_core/{archive_limits,install_record,error_codes}.h 与对应 ArkTS 镜像
 * [OUTPUT]: 断言 constexpr 头文件常量与 ArkTS export const 镜像逐值一致，漂移即失败退出
 * [POS]: harmony/pakta/src/test 的纯 Node 守门脚本，PR 上防止跨平台常量静默漂移
...
const pairs = [
  {
    ...
    mirror: 'harmony/pakta/src/main/ets/ArchiveLimits.ts',
    mirrorName: (cppName) => screamingSnake(cppName),
  },
  {
    header: 'cpp/patch_core/install_record.h',
    mirror: 'harmony/pakta/src/main/ets/InstallRecord.ts',
    mirrorName: (cppName) =>
      ({
  ...
  },
  {
    ...
    mirror: 'harmony/pakta/src/main/ets/ErrorCodes.ts',
