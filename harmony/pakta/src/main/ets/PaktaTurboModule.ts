/**
 * [INPUT]: 依赖 RNOH UITurboModule、UIAbility/bundleManager、Logger/EventHub、NativePatchCore、UpdateContext、JS 冻结的启动版本身份与冷启动键
 * [OUTPUT]: 对外提供 Pakta TurboModule 的常量（含不可变 channel）、版本状态、下载/切换/重载、版本绑定成功确认、配置缓存、bundleHash 与事件协议
 * [POS]: Harmony JS-Native 应用服务门面，统一参数校验和错误映射，所有持久化/补丁行为委托 UpdateContext
...
export class PaktaTurboModule extends UITurboModule {
  public static readonly NAME = 'Pakta';
  ...
  constructor(protected ctx: UITurboModuleContext) {
    ...
    logger.debug(TAG, ',PaktaTurboModule constructor');
  ...
  getConstants(): Object {
    ...
    const channel = this.context.getChannel();
  ...
  // 以下方法在 JS spec(src/NativePakta.ts)里都返回 Promise,C++ 方法表
  // (PaktaTurboModule.cpp)相应用 PAKTA_ASYNC_METHOD 注册:同步注册会把
  ...
  async markSuccess(expectedHash: string): Promise<boolean> {
    logger.debug(TAG, ',call markSuccess');
    try {
      return await this.context.markSuccess(expectedHash);
    } catch (error) {
