/**
 * [INPUT]: 依赖 patch_core/digest/HBC 能力、POSIX 文件系统与第三方 hpatch 执行器
 * [OUTPUT]: 实现 manifest 校验、安全复制合并、HBC 变换域补丁、旧版本清理与 hdiff 适配
 * [POS]: patch_core 的有副作用执行核心，是 Android/iOS/Harmony 文件 IO 语义的共享实现
