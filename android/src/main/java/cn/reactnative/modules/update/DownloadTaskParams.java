/**
 * [INPUT]: 依赖 File、UpdateContext.DownloadFileListener 与调用方给出的任务身份/截止时间
 * [OUTPUT]: 对内提供清理、全量、APK 差分、PPK 差分、普通下载五类任务命令对象
 * [POS]: UpdateContext 与 DownloadTask 之间的参数信封，集中表达文件目标和回调而不执行副作用
